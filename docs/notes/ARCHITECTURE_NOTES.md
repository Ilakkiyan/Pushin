...
- **Staleness is per-day, not per-moment.** A block is stale once it ends before **midnight of today**.
  A block you blew past at 9am is therefore *kept* until the day turns over, so the calendar never
...
window, `place` returned nothing, and the task got **no block at all**, so overdue work silently fell off
...
- ⚠️ **Iroh runs `1.x`. The old `0.90` pin silently broke all cross-network sync.** n0's relays stayed on the 0.90 protocol only until 0.92 shipped; 0.91 rebuilt relaying on standard HTTP, so an 0.90 client still *connects* to a relay and can no longer *route* through one. The symptom is brutal to diagnose because nothing errors: the relay handshake succeeds, registration looks healthy, and every dial then times out identically whether the peer is offline, firewalled, or fine. Pairing was impossible for anyone not on the same LAN, for the whole life of the 0.90 pin.
  - The pin existed because iroh 1.x pulls `netwatch` → `wmi 0.18.4`, and `wmi` declares BOTH `windows` and `windows-core` as `>=0.59, <0.63`, a range spanning two incompatible groups. Cargo satisfied the halves from *different* groups (`windows 0.61` unified with Tauri, `windows-core 0.62` unified with netwatch), so wmi compiled 0.61's COM types against 0.62's `Interface` trait: "the trait bound `IWbemObjectSink: Interface` is not satisfied". It reads like an unfixable dependency conflict and is really a resolver artifact. Naming `windows = "0.62"` for Windows targets in `Cargo.toml` puts it unambiguously in the graph so wmi resolves a matched pair. A stale `Cargo.lock` will keep the broken pairing, so regenerate it.
  - **1.x API shifts** (all inside `transport.rs` by design, plus call sites): `NodeAddr`→`EndpointAddr { id, addrs }`, `NodeId`→`EndpointId`, `Endpoint::node_id()`→`id()`, `node_addr().initialized()`→`addr()` (no longer a Result), `conn.alpn()` returns `&[u8]` not `Option`, and `Endpoint::builder()` takes a **preset** (`presets::N0` / `presets::N0DisableRelay`) instead of `relay_mode` + `discovery_n0`. There is no single "home relay is up" future any more; poll `ep.addr()` for a `TransportAddr::Relay`.
  - **Pairing invites are not compatible across this change** (the ticket serialises the address type), and neither is the wire protocol. Both devices must be on the same side of it.
  - Needs rusqlite `functions` + `data-encoding`. See memory `device-sync`.
...
- **Deterministic uuids.** `vault_file_index.uuid` is derived from the path (`db::vault_file_uuid` = `'vf-' || hex(rel_path)`), not random. Same lesson as `ICS_UUID_BACKFILL`. Two already-paired devices holding the same attachment would otherwise each mint a random id, and applying the peer's row would hit `UNIQUE(rel_path)` and abort the **entire** changeset batch (the `9ae15ce` failure mode).
...
- **Convergence rests on `vault_file_seen`.** It holds only paths *this* device has actually had on disk, which is what separates "the peer's file, not fetched yet" from "a file I deleted". Get that wrong and the index row a peer just created gets tombstoned by the receiver's next scan, and the file deletes itself moments after arriving.
...
- **The closing `done` is emitted on error paths too** (`sync_with` bookends the whole attempt; `sync_google` emits before the `?`). A peer that goes unreachable mid-session would otherwise leave the bar stuck at whatever fraction it reached with nothing left running to finish it. `App.tsx` adds a 30s watchdog for the case nothing can emit at all, such as a hard backend crash.
...
- **Release-pipeline gotchas (hard-won; releases run unattended, and a red ❌ is invisible unless you look):**
  ...
  - **macOS-only Rust break.** `netwatch` (via iroh) uses socket2's `all`-gated `Type::RAW` in its macOS/BSD netmon, so only macOS compiles it. **Fixed** with `socket2 = { version = "0.5", features = ["all"] }`. Verify without a Mac: `rustup target add aarch64-apple-darwin` then `cargo check --target aarch64-apple-darwin`.
...
- **Folders + the Drive-style browser (`VaultBrowserPane.tsx`, `lib/pageTree.ts`, migration `0024_page_folders`):** a folder is a `notes` row with `is_folder = 1` and no body, so it gets the same tree and the same `move_page`/delete/sort plumbing, so nothing about drag-reparent or device sync needed teaching. Because it *is* a page row, the discipline is subtractive: folders are excluded from `search_pages`, `title_index` (so `[[Work]]` can't resolve to a folder named Work), `page_graph` and `entities_for_index`. `create_folder` + `rename_page` are their own commands, because `update_page` rewrites the body and embedding, so renaming through it would blank a folder and force a full document round-trip on a page.
  - **The Journal is a VIRTUAL folder** (`JOURNAL_ID = -1`), not a row. Daily notes are identified by `daily_date` and created parentless by `get_or_create_daily`; giving them a physical parent would mean teaching `daily_note`, the file mirror and the graph about it. It's always shown at the root: a folder that only appears once you've used the thing it holds is one nobody finds, so the root is never an empty grid and the first-run nudge rides alongside the listing instead of replacing it.
  - **Two entry points, two questions:** the sidebar's Vault button and `g v` land on `files` (the browser, rooted) for "where is it?", while `vault` is the editor for "what was I writing?". Opening a *folder* browses; opening a *document* hands off to the editor.
  - **`openPageIds`** is the sidebar's "Open" switcher: session-only, most-recent-last, fed by the single `openState` helper that every route into the editor goes through, so a new entry point can't leave the switcher lying about what's open. Closing the active page falls back to its neighbour; closing the last drops back to the browser.
...
- **Wired:** planner auto-recall (pages-only, semantic-only, `RECALL_FLOOR` **0.65**, since bge-small's unrelated short-text baseline is ~0.59) and `vault_ask` (tasks/events/pages/people). Cmd-K is notes-only by choice.
