...
- **Folders + the Drive-style browser (`VaultBrowserPane.tsx`, `lib/pageTree.ts`, migration `0024_page_folders`):** a folder is a `notes` row with `is_folder = 1` and no body — same tree, same `move_page`/delete/sort plumbing, so nothing about drag-reparent or device sync needed teaching. Because it *is* a page row, the discipline is subtractive: folders are excluded from `search_pages`, `title_index` (so `[[Work]]` can't resolve to a folder named Work), `page_graph` and `entities_for_index`. `create_folder` + `rename_page` are their own commands — `update_page` rewrites the body and embedding, so renaming through it would blank a folder and force a full document round-trip on a page.
  ...
  - **Two entry points, two questions:** the sidebar's Vault button and `g v` land on `files` (the browser, rooted) — "where is it?"; `vault` is the editor — "what was I writing?". Opening a *folder* browses; opening a *document* hands off to the editor.
