...
1. **Universal Capture & Router**: one surface (text, voice→on-device STT later, web clipper,
   ...
   Extends `QuickCapture`/Inbox + the parser. *Reads:* none. *Writes:* all entity kinds. *LLM:* parse+route.
...
|---|---|---|---|
| Universal Capture | none | tasks/events/notes/people/goals | parse + route |
...
| Ask-your-life | everything (RAG) | none | synthesize + cite |
