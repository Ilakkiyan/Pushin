...
- **The Journal is a virtual folder** (`JOURNAL_ID = -1`), not a row. Daily notes are keyed by
  ...
  present at the root, since a folder that appears only once you have used the thing it holds is one
...
- **Two entry points, two questions.** The sidebar's Vault button and `g v` land on `files` (the
  browser, rooted) for "where is it?", while `vault` is the editor for "what was I writing?". Opening a folder
...
`free_slots`, because a hand drag is an explicit instruction, so it may land in the evening or the sleep
...
writing that preference is guarded, since a private window or blocked site data throws on access, and the
...
oldest rather than throwing). Cards for v0.8.1 and v0.8.2 were backfilled from this log, because the list had
...
and the forced dev preview (`?whatsnew=1`) still shows the full list, since filtering it against itself
...
- **.ics feeds now replicate** (`3d888ea`, migration `0022`). Import a calendar on one device and
  ...
  minted a random uuid, so the id is now derived from the feed URL so they converge), and a latent
...
dropped), so it went on the wire as a raw *local* rowid. With `foreign_keys = ON`, a peer holding no
...
- **The real bug underneath it:** `schedule_one` capped placement at the task's deadline. Once that
  ...
  overdue task got **no block at all**, so it silently vanished from the calendar exactly when it most
...
- **Also in this release**, already on `main` since v0.8.0: the Playwright smoke-suite repair and the
  crash it exposed (`01b43b1`, since v0.8.0 shipped with that suite red), a month-old miss now reads as
...
- **Parser:** a fuzzy `updateEvents` match no longer rewrites a whole series (the "updated 17 events"
  bug, since `best_update_target` picks the single best), timed comma lists get chunked, crammed
...
*before* investing in UI/scaffold: does the synced Rust core build for a phone? **Yes**, producing
...
Run Pushin on multiple devices, synced **without a cloud**, over a private peer-to-peer mesh (Iroh QUIC,
...
- **Phase 4 plan:** 4.1 focus tracking (done) → 4.2 adaptive scheduler (learn real durations, feeds
  estimates, which touches the scheduler IP, its own slice) → 4.3 Meeting Companion (brief over People +
...
- **Badge:** Sidebar now shows `AI ready · Memory ✓` / `· Memory…` (store `embedReady`, set from
  `ensure_embeddings`' result, which cheap-early-returns when healthy, so it doubles as a status probe).
...
**Empirical finding:** the two *unrelated* junk notes in the corpus had cosine **0.587**, which is bge-small's
...
- `RECALL_FLOOR` 0.35 → **0.65**, to clear bge-small's ~0.59 unrelated baseline with margin.
...
- `parser::store_plan` now **drops a task that fuzzy-matches an event created/updated the same turn**
  (the explicit calendar block is the intent), killing the double-booking. Deterministic; 2 unit tests.
