...
## Item A: Route users to the tuned model  *(highest leverage, smallest effort, so do it first)*
...
**Verify:** `cargo test --lib` + `npx tsc --noEmit` + `npm test` green; then a **live** run on a fresh
...
**Dependency:** ships *through* Item A's routing, so a better tuned GGUF just replaces the HF asset + a new
...
**already deterministic** (`scheduler.rs` docstring: "Deterministic, explainable, fast"), so it can hand
