  ...
  return (
    <div className="fade-in fixed inset-0 z-50 bg-black/50 flex items-start justify-center pt-[20vh]" onClick={() => setOpen(false)}>
      <div className="pop-in w-full max-w-lg mx-4 rounded-xl bg-[var(--raised)] border border-white/10 shadow-2xl overflow-hidden" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center gap-2 px-4 py-2 border-b border-white/10 text-sm text-[var(--ink-muted)]">
          <Inbox className="size-4 text-indigo-400" /> Quick capture
          <span className="ml-auto flex items-center gap-1.5 text-[10px] text-[var(--ink-faint)]">
        ...
        <div className="flex items-center justify-end gap-2 px-4 py-2 border-t border-white/10">
          <button onClick={() => setOpen(false)} className="text-xs px-3 py-1.5 hoverable text-[var(--ink-muted)] hover:text-white">
            Cancel
          </button>
          <button
            onClick={save}
            disabled={!text.trim() || saving}
            className="flex items-center gap-1.5 text-xs px-3 py-1.5 bg-white/90 hover:bg-white text-gray-900 font-medium disabled:opacity-40"
          >
