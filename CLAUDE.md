...
- **Release-pipeline gotchas (hard-won; every release v0.3.1→v0.6.0 silently failed before these were
  found — releases run unattended, so a red ❌ in the Actions tab is invisible unless you look):**
  - **Lockfile npm-major drift.** `npm ci` (CI) is strict: it fails `EUSAGE — Missing: react@18.3.1 …
    from lock file` when `package-lock.json` was generated by a *different npm major* than CI's. Dev
    Node 24 ships **npm 11** (dedupes a transitive react-18 peer subtree out of the lock); old CI Node 20
    ships **npm 10** (requires it). **Fixed** by pinning CI to the dev Node via **`.nvmrc` (24)** +
    `node-version-file: '.nvmrc'` in `release.yml`/`test.yml`, so the lock can't drift across npm majors.
    Repro a CI `npm ci` failure locally with `docker run --rm -v "$PWD:/src" node:20 sh -c "cp /src/package*.json . && npm ci"`.
  - **macOS-only Rust break.** `netwatch` (via **iroh**) uses socket2's `all`-gated `Type::RAW` +
    `Socket→UnixStream` in its **macOS/BSD** netmon (`netmon/bsd.rs`) — only macOS compiles that file, so
    Windows/Linux build fine while macOS fails. **Fixed** with `socket2 = { version = "0.5", features =
    ["all"] }` in `Cargo.toml` (feature-unifies into netwatch's socket2 0.5.10). Verify a macOS-only
    compile **without a Mac**: `rustup target add aarch64-apple-darwin` then `cargo check --target
    aarch64-apple-darwin` (type-checks the `cfg(macos)` paths; stops at objc2's `cc` step, which only CI has).
  - **Universal macOS binary.** Ship **one** `--target universal-apple-darwin` (native on Apple Silicon
    AND Intel) instead of separate arch builds, so installs never trip macOS's "Intel app" deprecation
    warning. The matrix is now 3 jobs (universal-mac / windows / linux); the toolchain step still installs
    both apple targets (universal = lipo of both).
  - Reading CI logs needs auth (`gh` or the API token); **job/step metadata is public** —
    `GET /repos/{o}/{r}/actions/runs/{id}/jobs` reveals *which step* failed without auth.
...
- **Working — shell + UX (v0.6.x):** full **grey-minimalist charcoal redesign** (`@theme` accent remap +
  ...
  **WelcomeGuide**, a **"What's new"** post-update intro (shown once per version bump), premium-but-fast
  motion. **Keyboard-first**: Linear-style **`g`-then-key** nav (`useHotkeys`) + ⌘K palette shortcuts;
  calendar **slot selection** (mouse + arrows, Enter/double-click to create); **Ctrl+T** → auto-titled,
  event-linked vault note. **Two-way markdown file vault** (Obsidian-style; migration 0016 + `notify`
  watcher + echo guard — see the vault section). **In-app auto-update** (v0.5.0+) with signed artifacts +
  `latest.json`. Booking page is **hidden from release builds** for now (`import.meta.env.DEV` gate).
