...
12. **Fabrication guards (`parser.rs::apply_recovery`, all deterministic + unit-tested).** Even the 7B fabricates. Three tightly-gated recovery passes make the AI **ask instead of guess** — judge each against the live model, they're conservative on purpose:
    ...
    - `drop_unsupported_edits` — the union prompt shows an edit AND a remove exemplar in **every** prompt and the system prompt lists the whole calendar, so on a plain statement of fact ("I also have physics class from 12:45 to 2 pm") the tuned 7B tacked on `updateEvents:[{match:"Gym"}]` → "updated 1 event (Gym)". Drops an update/remove whose target the user never typed, but ONLY when the message has no edit verb and no back-reference pronoun ("move the dentist to 3pm" / "make it 2 hours" are untouched).
    - `drop_parroted_exemplars` — the model also emits few-shot titles verbatim: a phantom "Lunch with mom" beside the real event, and one *created* on "Cancel lunch with Dan". Drops an event/habit/task whose title is a known `EXEMPLAR_TITLES` entry sharing **no** significant word with the message, and only while a genuine sibling survives (never turn a turn into a no-op). The any-word test is deliberate: on "Move lunch to 1pm" the model dresses the user's real lunch in the exemplar title and `route_edit_verb_creates_to_updates` still needs it.
    ⚠️ **Both of these matter twice over.** `backfill_event_fields` only recovers times for an *unambiguous single create* (`single_create`), so ANY fabricated sibling — a phantom edit, a parroted exemplar, a split fragment — silently disables time recovery and the event the user really described falls back to the **60-minute default**. That is how "physics class from 12:45 **to 2 pm**" became a 12:45–13:45 block. Anything that can add a spurious item must run BEFORE the backfill.
...
- **Windows dev box** (primary as of 2026-08-25): Node 22/npm 11, Rust 1.98 `stable-x86_64-pc-windows-msvc` + VS 2022 Build Tools. `cargo` is NOT on PATH — prefix `$env:PATH = "$env:USERPROFILE\.cargo\bin;$env:PATH"`. Headless inference (no GUI needed; a launched app reuses an already-listening server):
  ```powershell
  $bin="$env:APPDATA\com.pushin.app\bin\llama-server.exe"; $m="$env:APPDATA\com.pushin.app\models"
  Start-Process $bin -ArgumentList @("-m","$m\pushin-arch7b-chat-tuned-q4_k_m.gguf","--host","127.0.0.1","--port","8080","-c","4096") -WindowStyle Hidden
  Start-Process $bin -ArgumentList @("-m","$m\bge-small-en-v1.5-q8_0.gguf","--host","127.0.0.1","--port","8181","-c","512","--embeddings") -WindowStyle Hidden
  ```
  **Always set `PUSHIN_EVAL_DEBUG=1`** on an eval run — the per-case `DBG event/task` dump names *which* item is wrong; the pass/fail line alone sent me hunting the wrong bug twice.
- **Screenshots — two harnesses that see different things.** `npm run capture` drives Chromium against the **mocked** bridge (`tests/e2e/_capture.spec.ts`) → great for UI, but it can show **nothing the Rust core computed**. `scripts/capture-window.ps1` screenshots the **real** Tauri window (PrintWindow, retried until the WebView paints, falling back to foreground+CopyFromScreen) → the only way to see real parser/scheduler output. Launch with `$env:VITE_PUSHIN_INTRO="0"` (or `?whatsnew=0` in-browser) or the dev "what's new" replay covers everything.
- ⚠️ Writing to `com.pushin.app/pushin.db` from a script needs a `sync_capturing` scalar fn registered on the connection — the `0015` change-capture triggers call it in their `WHEN` clause and the write aborts without it. Close the app first (it holds the DB), and copy **db + -wal + -shm together** or you'll read a stale snapshot (the main file is often ~1 page; everything lives in the WAL).
